...
function initialForm(listing: Listing | undefined): ListingForm {
  ...
  return {
    ...
    price: listing?.price
      ? String(fromMinorUnits(listing.price, listing?.currency || 'gbp'))
      : '',
...
export function EditListingPage() {
      ...
      (detail?.shipping ?? []).map((opt) => ({
        ...
        price: opt.price
          ? String(
              fromMinorUnits(
                opt.price,
                opt.currency || listing?.currency || 'gbp'
              )
            )
          : '',
  ...
  const [auctionDuration, setAuctionDuration] = useState(
    relistInit?.duration ?? '7'
  )
  const relistCurrency = listing?.currency || 'gbp'
  const [reserve, setReserve] = useState(
    relistInit?.reserve
      ? String(fromMinorUnits(relistInit.reserve, relistCurrency))
      : ''
  )
  const [instantBuy, setInstantBuy] = useState(
    relistInit?.instant
      ? String(fromMinorUnits(relistInit.instant, relistCurrency))
      : ''
  )
  ...
  function changeType(next: ListingType) {
    ...
    setForm((f) => {
      ...
      const keep =
        !category ||
        (next === 'digital' ? category.digital === 1 : category.physical === 1)
  ...
  const startPriceMinor =
    form.pricing === 'auction' && form.price
      ? toMinorUnits(form.price, form.currency)
      : 0
  ...
  const reserveBelowStart =
    ...
    reserveMinor > 0 &&
    startPriceMinor > 0 &&
    reserveMinor <= startPriceMinor
  const instantBelowStart =
    form.pricing === 'auction' &&
    instantMinor > 0 &&
    startPriceMinor > 0 &&
    instantMinor <= startPriceMinor
  const reserveAboveInstant =
    form.pricing === 'auction' &&
    reserveMinor > 0 &&
    instantMinor > 0 &&
    reserveMinor > instantMinor
  ...
  const minDisplay = formatPrice(
    currencyMinimum(form.currency, fees),
    form.currency
  )
  const canPublish =
    missing.length === 0 &&
  ...
  return (
    <>
      <PageHeader
        ...
        back={{
          label: t`My listings`,
          onFallback: () => navigate({ to: APP_ROUTES.LISTINGS.MINE }),
        }}
        actions={
        ...
        <div className='max-w-2xl space-y-6'>
          {isDraft && !isOnboarded && (
            <div className='flex items-center justify-between gap-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm'>
              <span>
                {stripeLinked ? (
                  <Trans>
                    Stripe needs more information. Complete the requirements on
                    your Stripe Dashboard to publish listings.
                  </Trans>
                ) : (
          ...
          {!isDraft && (
            <div className='rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm'>
              <Trans>
                This listing is{' '}
                {STATUS_LABELS[listing.status] ?? listing.status}. Editing is
                disabled.
              </Trans>
            </div>
          )}
          ...
          >
            <section className='bg-card space-y-4 rounded-lg border p-4 sm:p-6'>
                  ...
                  {form.pricing !== 'auction' && (
                    <div className='space-y-2'>
                      ...
                      <Input
                        ...
                        min='1'
                        placeholder={
                          unlimitedStock ? t`Unlimited` : t`Number of units`
                        }
              ...
              <div className='grid gap-4 sm:grid-cols-2'>
                ...
                <div className='space-y-1.5'>
                  ...
                  <Input
                    id='price'
                    inputMode={
                      currencyDecimals(form.currency) === 0
                        ? 'numeric'
                        : 'decimal'
                    }
                    value={form.price}
                    ...
                    onChange={(e) => {
                      ...
                      if (val !== '' && !priceRegex(form.currency).test(val))
                        return
                      update('price', val)
                    }}
                  />
                ...
                <div className='grid gap-4 sm:grid-cols-2'>
                  ...
                  <div className='space-y-1.5'>
                    <Label htmlFor='reserve'>
                      {currencySymbol
                        ? t`Reserve price (${currencySymbol})`
                        : t`Reserve price`}
                    </Label>
                    <Input
                      id='reserve'
                    ...
                    {reserveBelowStart && (
                      <p className='text-destructive text-xs' role='alert'>
                        <Trans>
                          Reserve price must be higher than the starting bid.
                        </Trans>
                      </p>
                    )}
                    {!reserveBelowStart && reserveAboveInstant && (
                      <p className='text-destructive text-xs' role='alert'>
                        <Trans>
                          Reserve price must not exceed the buy it now price.
                        </Trans>
                      </p>
                    )}
                  </div>
                  <div className='space-y-1.5'>
                    <Label htmlFor='instant'>
                      {currencySymbol
                        ? t`Buy it now price (${currencySymbol})`
                        : t`Buy it now price`}
                    </Label>
                    <Input
                      id='instant'
                    ...
                    {instantInvalid && (
                      <p className='text-destructive text-xs' role='alert'>
                        <Trans>
                          Buy it now price must be higher than the starting bid.
                        </Trans>
                      </p>
                    )}
                  </div>
                ...
                {(assets.length > 0 || uploadingAssets > 0) && (
                  <div className='space-y-2' {...assetOrder.getGroupProps()}>
                    ...
                    {Array.from({ length: uploadingAssets }).map((_, i) => (
                      <div
                        key={`uploading-${i}`}
                        className='text-muted-foreground flex items-center gap-3 rounded-lg border border-dashed p-3 text-sm'
                      >
                        <Loader2 className='size-4 animate-spin' />
                        <span>
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          toast.error(
                            getErrorMessage(
                              err,
                              t`Failed to add external asset`
                            )
                          )
                        }
                      }}
                ...
                {form.shipping && (
                  <div className='space-y-1.5'>
                          ...
                          >
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                if (
                                  val !== '' &&
                                  !priceRegex(opt.currency).test(val)
                                )
                                  return
                    ...
                    >
                      ...
                      <Trans>Add shipping option</Trans>
                    </Button>
                  </div>
                )}
          ...
          <ResponsiveDialogContent>
            ...
            <div className='space-y-4 py-2'>
              <Card className='rounded-lg'>
                <CardContent className='space-y-1 p-3 text-sm'>
                  ...
                  <div className='text-muted-foreground'>
                    ...
                    {
                      PRICING_MODELS.find((p) => p.value === form.pricing)
                        ?.label
                    }
                  </div>
                </CardContent>
              ...
              <p className='text-muted-foreground text-sm'>
                <Trans>
                  When you publish, your listing goes through an automated
                  check. Most listings go live immediately; some are held for
                  Mochi staff to review before they become visible.
                </Trans>
              </p>
            </div>
            <ResponsiveDialogFooter>
...
function publishMissing(form: ListingForm): string[] {
  ...
  if (form.pricing === 'subscription' && !form.interval)
